...
const getAllPlaces = async (req, res) => {
    try {
        const places = await placeModel.find().populate("usersids", "name").populate("subplaces", "name");
        return places;
    } catch (error) {
...
const getPlaceById = async (placeid) => {
    try {
        const place = await placeModel.findById(placeid).populate("usersids", "name").populate("subplaces", "name");
        if (!place) {
...
const getplaceByName = async (placename) => {
    try {
        const place = await placeModel.findOne({name: placename}).populate("usersids", "name").populate("subplaces", "name");
        if (!place) {
...
const updateplace = async (placeid, placeData) => {
    try {
        const place = await placeModel.findByIdAndUpdate(placeid, placeData, { new: true }).populate("usersids", "name").populate("subplaces", "name");
        if (!place) {
...
const updateUsers = async (userid) => {
    try {
        ...
        const place = await placeModel.findOneAndUpdate({},{$addToSet: {usersids: userid}}, {new: true});
        await placeModel.findOneAndUpdate({},{$set: {users: place.usersids.length}}, {new: true});
    } catch (error) {
        throw new Error("Chyba pri pridávaní používateľa: " + error.message);
    }
};
...
const updateSubplaces = async (subplaceid) => {
    try {
        ...
        await placeModel.findOneAndUpdate({},{$addToSet: {subplaces: subplaceid}}, {new: true});
    } catch (error) {
        throw new Error("Chyba pri pridávaní subplace: " + error.message);
    }
};
...
const deleteSubplaces = async (subplaceid) => {
    try {
        ...
        const place = await placeModel.findOneAndUpdate({},{$pull: {subplaces: subplaceid}}, {new: true});
        await placeModel.findOneAndUpdate({}, {$set: {subplacesCount: place.subplaces.length}}, {new: true});
    } catch (error) {
        throw new Error("Chyba pri vymazávaní subplace: " + error.message);
    }
};
...
const getPlace = async () => {
    try {
        const place = await placeModel.find({}).populate("usersids", "name").populate("subplaces", "name");
        if (!place) {
            throw new Error("Place nebol nájdený");
        }
        return place;
