...
const getAllSubplaces = async (req, res) => {
    try {
        const subplaces = await subplaceModel.find().populate("moderators", "name").populate("creator", "name").populate("questions", "title").populate("membersids", "name");
        return subplaces;
    } catch (error) {
...
const getSubplaceById = async (subplaceid) => {
    try {
        const subplace = await subplaceModel.findById(subplaceid).populate("membersids", "name").populate("creator", "name").populate("moderators", "name").populate("questions", "title");
        if (!subplace) {
...
const getSubplaceByName = async (subplacename) => {
    try {
        const subplace = await subplaceModel.findOne({name: subplacename}).populate("membersids", "name").populate("creator", "name").populate("moderators", "name").populate("questions", "title");
        return subplace;
    } catch (error) {
...
const updateSubplace = async (subplaceid, subplaceData) => {
    try {
        const subplace = await subplaceModel.findByIdAndUpdate(subplaceid, subplaceData, { new: true }).populate("membersids", "name").populate("creator", "name").populate("moderators", "name").populate("questions", "title");
        if (!subplace) {
...
const updateModerators = async (userid, subplaceid) => {
    try {
        ...
        const moderators = await subplaceModel.findByIdAndUpdate(subplaceid, {$addToSet: {moderators: userid}}, {new: true}).populate("membersids", "name").populate("creator", "name").populate("moderators", "name").populate("questions", "title");
        return moderators;
    } catch (error) {
...
const deletemoderator = async (userid, subplaceid) => {
    try {
        ...
        const moderators = await subplaceModel.findByIdAndUpdate(subplaceid, {$pull: {moderators: userid}}, {new: true}).populate("membersids", "name").populate("creator", "name").populate("moderators", "name").populate("questions", "title");
        return moderators;
    }
    catch (error) {
...
const updateQuestions = async (questionid,subplaceid) => {
    try {
        const updateQuestions = await subplaceModel.findByIdAndUpdate(subplaceid, {$addToSet: {questions: questionid } }, {new: true});
        if (!updateQuestions) {
            throw new Error("Chyba pri aktualizovaní otázky pre subplace " + error.message)
        }
    } catch (error) {
        throw new Error("Chyba pri aktualizovaní otázky pre subplace " + error.message);
    }
...
const deleteQuestion = async (questionid, subplaceid) => {
    try {
        const updateQuestions = await subplaceModel.findByIdAndUpdate(subplaceid, {$pull: {questions: questionid}}, {new: true});
        if (!updateQuestions) {
            throw new Error("Chyba pri aktualizovaní otázky pre subplace " + error.message)
        }
    } catch (error) {
...
const checkIfUserInSubplace = async (memberid, subplaceid) => {
    try {
        ...
        const foudnuser = subplace.membersids.some(id => id.toString() === memberid.toString());
        if (!foudnuser) {
            return null;
        }
        return foudnuser;
    } catch (error) {
        throw new Error("Chyba pri aktualizovaní členov subplaces " + error.message);
    }
};

const updateMembers = async (memberid, subplaceid) => {
    try {
        const subplace = await subplaceModel.findByIdAndUpdate(subplaceid, {$addToSet: {membersids: memberid}}, { new: true});
    } catch (error) {
        throw new Error("Chyba pri aktualizovaní členov subplaces " + error.message);
    }
};
...
const deleteMember = async (memberid, subplaceid) => {
    try {
        const subplace = await subplaceModel.findByIdAndUpdate(subplaceid,{$pull: {membersids: memberid}}, {new: true});
        if (!subplace) {
            throw new Error("Subplace neexistuje");
        }
        await subplaceModel.findByIdAndUpdate(subplaceid, {$set: {members: subplace.membersids.length}}, {new: true});
    } catch (error) {
