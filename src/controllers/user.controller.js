...
const { createUser, getUserById, getUserByUsername, updateUser, deleteUser, getAllUsers, getUserByEmail, checkUserPassword, updateUserSubplaces } = require("../../src/repository/user.repository");
...
const loginUserController = async (req, res) => {
    try {
        ...
        const matchedpassword = checkUserPassword(req.body.password, user.password, user.salt)
        if (!matchedpassword) {
...
const getAllUsersController = async (req, res) => {
    try {
        ...
        return res.status(500).json({ message: 'Chyba pri získavaní používateľov' });
    }
}
