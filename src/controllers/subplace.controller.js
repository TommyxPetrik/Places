...
const createSubplace = async (req, res) => {
    try {
        ...
        const subplaceDB = await subplaceRepository.getSubplaceByName(subplace.name);
...
const updateSubplace = async (req, res) => {
    try {
        const subplace = await subplaceRepository.updateSubplace(req.query.id, req.body);
        if (!subplace) {
            return res.status(404).json({ message: "Subplace not found" });
        }
        res.status(200).json(subplace);
...
const joinSubplace = async (req, res) => {
    try {
        const existing = await subplaceRepository.checkIfUserInSubplace(req.user.userid, req.query.subplaceid);
        if (!existing) {
            await subplaceRepository.updateMembers(req.user.userid, req.query.subplaceid);
            await userRepository.updateUserSubplaces(req.user.userid, req.query.subplaceid);
        ...
        res.status(400).json("Chyba pri pridaní požívateľa do subplace: " + error.message);
    }
};

const leaveSubplace = async (req, res) => {
    try {
        const isCreator = await subplaceRepository.checkIfUserIsCreator(req.user.userid, req.query.subplaceid);
        if (isCreator) {
            return res.status(400).json("Nemôžeš opustiť subplace, pretože si jeho zakladateľ");
        }
        const existing = await subplaceRepository.checkIfUserInSubplace(req.user.userid, req.query.subplaceid);
        if (!existing) {
            res.status(400).json("Pužívateľ nie je členom subplace");
        }
        await subplaceRepository.deleteMember(req.user.userid, req.query.subplaceid);
        res.status(200).json("Používateľ bol úspešne vymazaný");
    } catch (error) {
        res.status(400).json("Chyba pri vymazavaní požívateľa zo subplace: " + error.message);
    }
};

const updateModerators = async (req, res) => {
    try {
        const subplace = await subplaceRepository.updateModerators(req.body.userid, req.body.subplaceid);
        if (!subplace) {
            return res.status(404).json({ message: "Subplace not found" });
        }
        res.status(200).json(subplace);
...
const deleteModerator = async (req, res) => {
    try {
        const subplace = await subplaceRepository.deletemoderator(req.body.userid, req.body.subplaceid);
        if (!subplace) {
            return res.status(404).json({ message: "Subplace not found" });
        }
        res.status(200).json(subplace);
...
const updateQuestions = async (req, res) => {
    try {
        const subplace = await subplaceRepository.updateQuestions(req.body.subplaceid, req.body.questionid);
        if (!subplace) {
            return res.status(404).json({ message: "Subplace not found" });
        }
        res.status(200).json(subplace);
...
const deleteQuestion = async (req, res) => {
    try {
        const subplace = await subplaceRepository.deleteQuestion(req.body.subplaceid, req.body.questionid);
        if (!subplace) {
            return res.status(404).json({ message: "Subplace not found" });
        }
        res.status(200).json(subplace);
