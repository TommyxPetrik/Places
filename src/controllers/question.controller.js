const { createQuestion, getQuestionById, updateQuestion, deleteQuestion, getQuestionsByTag, getAllQuestions} = require("../repository/question.repository");
...
const createQuestionController = async (req, res) => {
    try {
        ...
        const newQuestion = {...req.body, userid: req.user.userid, username: req.user.username};
...
const getAll = async (req, res) => {
    try {
        ...
        return res.status(500).json({ message: 'Chyba pri získavaní otázok ' + error.message});
    }
};
...
const updateQuestionController = async (req, res) => {
    try {
        ...
        if (question.userid.id.toString() !== id.toString()) {
            return res.status(403).json("Nemáte správne oprávenia uprávovať túto otázku")
        }
